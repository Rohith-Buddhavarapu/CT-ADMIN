...
const BMSManager: React.FC<BMSManagerProps> = ({ issues, setIssues, user }) => {
  ...
  const handleAddIssue = async () => {
    ...
    const priority = (validPriorities.find(p => suggestedPriority.toLowerCase() === p.toLowerCase()) as Priority) || Priority.MEDIUM;
          ...
          <div key={issue.id} className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 group overflow-hidden">
            <div className="flex justify-between items-start mb-4 md:mb-6">
              ...
              <div className="text-right">
                <div className={`px-2 md:px-3 py-1 md:py-1.5 rounded-lg md:rounded-xl border text-[8px] md:text-[9px] font-black uppercase tracking-widest mb-1 ${
